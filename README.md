...
const KPIModalContent = React.memo(({ node, data }) => {
    const hardcodedData = {
      ...
      const processData = (techType , kpiData) => {
        ...
        const mergedData = kpiList.map((kpiName) => {
            const apikpi = kpiData.find(kpi => kpi.kpi === kpiName);
            console.log('apikpi');
            console.log(apikpi);
            return apikpi || {kpi: kpiName , succ:null, avg:null, att:null, last_7_att:null };
        });
        const remainingApiData = kpiData.filter(kpi => !kpiList.includes(kpi.kpi));
        console.log('mergedData');
        console.log(mergedData);
        return[...mergedData, ...remainingApiData];
      };

      const processedData = processData(node.nodetype , data.kpi);
            ...
            <Table style={styles.tableTop}>
                ...
                <TableBody>
                    <TableRow>
                        ...
                        <TableCell style={getRCStyle(node?.stats?.RC_Value?.att || node?.stats?.RC?.att)}>{ node?.stats?.RC_Value?.att || node?.stats?.RC?.att || 0}</TableCell>
