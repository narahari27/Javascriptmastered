...
export const NodeProvider = ({ children }) => {
  ...
  const processFilters = (filters) => {
    if (Object.keys(filters).length) {
      ...
      if (!oor) {
        filtered = filtered?.filter(_ => (_.nestStatus?.toLowerCase() === 'inservice' || _.nestStatus?.toLowerCase() === 'not_found'));
  ...
  const toggleLoader = (duration = 30000)=>{
    setDataLoading(true);
    setTimeout(()=>{
      setDataLoading(false);
    },duration);
